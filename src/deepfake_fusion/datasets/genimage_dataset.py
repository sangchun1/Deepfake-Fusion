...
def get_project_root() -> Path:
    """repo root 반환.
    현재 파일 위치: src/deepfake_fusion/datasets/genimage_dataset.py
    """
    return Path(__file__).resolve().parents[3]


class GenImageDataset(Dataset):
    """GenImage binary classification dataset.

    기대하는 CSV 형식(최소):
    filepath,label

    예시:
    data/raw/genimage/Midjourney/train/ai/000001.png,1
    data/raw/genimage/Midjourney/train/nature/000123.JPEG,0

    추가 메타컬럼이 있어도 무방:
    filepath,label,label_name,generator,generator_slug,official_split
    ...
    def _resolve_image_path(self, filepath: str) -> Path:
        ...
        2) project root 기준 상대경로
           예: data/raw/genimage/Midjourney/train/ai/000001.png
        3) root_dir 기준 상대경로
           예: Midjourney/train/ai/000001.png
        """
    ...
    def _normalize_label(label: Any) -> int:
        ...
        if label_str in {"0", "real", "nature"}:
            return 0
        if label_str in {"1", "fake", "ai"}:
            return 1
